...
class OpennetworkIdenticalRefunds(BitcoinTestFramework):
    def __init__(self):
        super().__init__()
    ...
    def run_test(self):
        ...
        self.node.sendtocontract(contract_address, "00", 0, 1000000, OPENNETWORK_MIN_GAS_PRICE/COIN, sender_address)
        self.node.sendtoaddress(sender_address, 1)
        self.node.sendtocontract(contract_address, "00", 0, 1000000, OPENNETWORK_MIN_GAS_PRICE/COIN, sender_address)
...
if __name__ == '__main__':
    OpennetworkIdenticalRefunds().main()
