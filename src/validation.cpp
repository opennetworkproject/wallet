...
#if defined(NDEBUG)
# error "Opennetwork cannot be compiled without assertions."
#endif
...
std::unique_ptr<OpennetworkState> globalState;
...
const std::string strMessageMagic = "Opennetwork Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > MAX_TRANSACTION_BASE_SIZE ||
        ::GetSerializeSize(tx, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // opennetwork
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = opennetworkDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            ...
            OpennetworkTxConverter converter(tx, NULL);
            ExtractOpennetworkTX resultConverter;
            if(!converter.extractionOpennetworkTransactions(resultConverter)){
            ...
            std::vector<OpennetworkTransaction> opennetworkTransactions = resultConverter.first;
            std::vector<EthTransactionParams> opennetworkETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(OpennetworkTransaction opennetworkTransaction : opennetworkTransactions){
                sumGas += opennetworkTransaction.gas() * opennetworkTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, opennetworkTransaction.gasPrice());
                } else {
                    txMinGasPrice = opennetworkTransaction.gasPrice();
                }
                VersionVM v = opennetworkTransaction.getVersion();
                if(v.format!=0)
                ...
                if(opennetworkTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(opennetworkTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += opennetworkTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)opennetworkTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(opennetworkETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > opennetworkTransactions.size())
        ...
        {
            LogPrint("mempool", "replacing tx %s with %s for %s OPENNETWORK additional fees, %d delta bytes\n",
...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // opennetwork
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // opennetwork

    if(pfClean == NULL && fLogEvents){
        boost::filesystem::path stateDir = GetDataDir() / "stateOpennetwork";
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    OpennetworkTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<OpennetworkTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path opennetworkDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(opennetworkDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(opennetworkDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(OpennetworkTransaction& tx : txs){
        //validate VM version
...
bool OpennetworkTxConverter::extractionOpennetworkTransactions(ExtractOpennetworkTX& opennetworktx){
    std::vector<OpennetworkTransaction> resultTX;
    ...
    opennetworktx = std::make_pair(resultTX, resultETP);
    return true;
}

bool OpennetworkTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool OpennetworkTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
OpennetworkTransaction OpennetworkTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    OpennetworkTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = OpennetworkTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = OpennetworkTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // opennetwork
    OpennetworkDGP opennetworkDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setOpennetworkSchedule(opennetworkDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = opennetworkDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = opennetworkDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = opennetworkDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // opennetwork
...
///////////////////////////////////////////////////////////////////////////////////////// opennetwork
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            OpennetworkTxConverter convert(tx, &view, &block.vtx);

            ExtractOpennetworkTX resultConvertOpennetworkTX;
            if(!convert.extractionOpennetworkTransactions(resultConvertOpennetworkTX)){
            ...
            if(!CheckMinGasPrice(resultConvertOpennetworkTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertOpennetworkTX.first, blockGasLimit);
            ...
            for(OpennetworkTransaction& qtx : resultConvertOpennetworkTX.first){
            ...
            if (fLogEvents)
            {
                for(size_t k = 0; k < resultConvertOpennetworkTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertOpennetworkTX.first[k].from(), resultConvertOpennetworkTX.first[k].to(),
...
bool static ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace)
{
    ...
    LogPrint("bench", "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * 0.001, nTimeReadFromDisk * 0.000001);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // opennetwork
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // opennetwork
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // opennetwork
            globalState->setRootUTXO(oldHashUTXORoot); // opennetwork
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // opennetwork
        uint32_t sizeBlockDGP = opennetworkDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Opennetwork PoS we need to sync this after every block to ensure txdb is populated for
