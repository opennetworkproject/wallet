...
bool ToHash160(const std::string& strOpennetworkAddress, std::string& strHash160)
{
    CBitcoinAddress opennetworkAddress(strOpennetworkAddress);
    if(opennetworkAddress.IsValid()){
        CKeyID keyid;
        opennetworkAddress.GetKeyID(keyid);
...
bool ToOpennetworkAddress(const std::string& strHash160, std::string& strOpennetworkAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CBitcoinAddress opennetworkAddress;
    opennetworkAddress.Set(keyid);
    if(opennetworkAddress.IsValid()){
        strOpennetworkAddress = opennetworkAddress.ToString();
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToOpennetworkAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToOpennetworkAddress(tokenEvent.receiver, tokenEvent.receiver);
