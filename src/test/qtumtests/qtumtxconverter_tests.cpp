...
void checkResult(bool isCreation, std::vector<OpennetworkTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    OpennetworkTxConverter converter(transaction, NULL);
    ExtractOpennetworkTX opennetworkTx;
    BOOST_CHECK(converter.extractionOpennetworkTransactions(opennetworkTx));
    std::vector<OpennetworkTransaction> result = opennetworkTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(opennetworktxconverter_tests, TestingSetup)
