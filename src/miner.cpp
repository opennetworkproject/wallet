...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setOpennetworkSchedule(opennetworkDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = opennetworkDGP.getBlockSize(nHeight);
    minGasPrice = opennetworkDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = opennetworkDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    OpennetworkTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractOpennetworkTX resultConverter;
    if(!convert.extractionOpennetworkTransactions(resultConverter)){
    ...
    for(OpennetworkTransaction opennetworkTransaction : opennetworkTransactions){
        txGas += opennetworkTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + opennetworkTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(opennetworkTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, opennetworkTransactions, hardBlockGasLimit);
...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("opennetworkcoin-miner");
...
void StakeOpennetworks(bool fStake, CWallet *pwallet)
{
