...
#include "opennetworkstate.h"

using namespace std;
using namespace dev;
using namespace dev::eth;

OpennetworkState::OpennetworkState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = OpennetworkState::openDB(_path + "/opennetworkDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

OpennetworkState::OpennetworkState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute OpennetworkState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, OpennetworkTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createOpennetworkAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            opennetwork::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> OpennetworkState::vins() const // temp
{
...
void OpennetworkState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* OpennetworkState::vin(dev::Address const& _a) const
{
    return const_cast<OpennetworkState*>(this)->vin(_a);
}

Vin* OpennetworkState::vin(dev::Address const& _addr)
{
...
// void OpennetworkState::commit(CommitBehaviour _commitBehaviour)
// {
...
void OpennetworkState::kill(dev::Address _addr)
{
...
void OpennetworkState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address OpennetworkState::createOpennetworkAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void OpennetworkState::deleteAccounts(std::set<dev::Address>& addrs){
...
void OpennetworkState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void OpennetworkState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
