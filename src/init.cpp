...
void Shutdown()
{
    ...
    RenameThread("opennetwork-shutoff");
    ...
    if (pwalletMain)
    {
        StakeOpennetworks(false, pwalletMain);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/opennetworkproject/opennetwork>";
    const std::string URL_WEBSITE = "<https://opennetwork.org>";
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Opennetwork version %s\n", FormatFullVersion());
}
...
//////////////////////////////////////////////////////////////////////
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                boost::filesystem::path opennetworkStateDir = GetDataDir() / "stateOpennetwork";
                bool fStatus = boost::filesystem::exists(opennetworkStateDir);
                const std::string dirOpennetwork(opennetworkStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsOpennetworkstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<OpennetworkState>(new OpennetworkState(dev::u256(0), OpennetworkState::openDB(dirOpennetwork, hashDB, dev::WithExisting::Trust), dirOpennetwork, existsOpennetworkstate));
    ...
    if (!GetBoolArg("-staking", DEFAULT_STAKE))
        ...
        StakeOpennetworks(true, pwalletMain);
#endif
    // ********************************************************* Step 12: finished
