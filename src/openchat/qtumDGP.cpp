#include "opennetworkDGP.h"

void OpennetworkDGP::initDataEIP158(){
...
bool OpennetworkDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule OpennetworkDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t OpennetworkDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t OpennetworkDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t OpennetworkDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t OpennetworkDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool OpennetworkDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void OpennetworkDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void OpennetworkDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void OpennetworkDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void OpennetworkDGP::createParamsInstance(){
...
dev::Address OpennetworkDGP::getAddressForBlock(unsigned int blockHeight){
...
void OpennetworkDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OpennetworkDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void OpennetworkDGP::parseStorageOneUint64(uint64_t& value){
...
void OpennetworkDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = uint64_t(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule OpennetworkDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
